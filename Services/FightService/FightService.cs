        ...
        public async Task<ServiceResponse<FightResultDto>> Fight (FightRequestDto request)
        {
            ...
            try
            {
                List<Character> characters = await _context.Characters
                    .Include (f => f.Weapon)
                    .Include (f => f.CharacterSkills).ThenInclude (f => f.Skill)
                    .Where (f => request.CharacterIds.Contains (f.Id))
                    .ToListAsync ();
                ...
                while (!defeated)
                {
                    ...
                    {
                        List<Character> opponents = characters.Where (f => f.Id != attacker.Id).ToList ();
        ...
        public async Task<ServiceResponse<AttackResultDto>> SkillAttack (SkillAttackDto request)
        {
            ...
            try
            {
                Character attacker = await _context.Characters
                    .Include (c => c.CharacterSkills).ThenInclude (f => f.Skill)
                    .FirstOrDefaultAsync (c => c.Id == request.AttackerId);

                Character opponent = await _context.Characters
                    .FirstOrDefaultAsync (f => f.Id == request.OpponentId);

                CharacterSkill characterSkill = attacker.CharacterSkills.FirstOrDefault (f => f.Skill.Id == request.SkillId);
